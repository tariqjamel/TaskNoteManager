...
class DataBaseHelper(context: Context, factory: SQLiteDatabase.CursorFactory?) :
    ...
    companion object {
        ...
        const val NOTE_TABLE = "notes" // New table for notes
        const val USER_ID = "idUser"
        const val TASK_ID = "_id"
        const val NOTE_ID = "_id" // New column for note ID
        const val NOTE_CONTENT = "content" // New column for note content
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        // New table for notes
        val CreateNoteTable = "CREATE TABLE $NOTE_TABLE ($NOTE_ID INTEGER PRIMARY KEY AUTOINCREMENT," +
                "$NOTE_CONTENT TEXT, $USER_ID INTEGER)"
    ...
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        ...
        db?.execSQL("DROP TABLE IF EXISTS $NOTE_TABLE") // Drop notes table on upgrade
        onCreate(db)
    }
    ...
    fun addNote(userId: Int, content: String) {
        val db = this.writableDatabase
        val query = "INSERT INTO $NOTE_TABLE ($NOTE_CONTENT, $USER_ID) " +
    ...
    fun getNotes(userId: Int): Cursor? {
        val db = this.readableDatabase
        return db.rawQuery("SELECT * FROM $NOTE_TABLE WHERE $USER_ID = ${userId}", null)
    }
    ...
    fun updateNote(note: Note) {
        val db = this.writableDatabase
        val query = "UPDATE $NOTE_TABLE SET $NOTE_CONTENT = '${note.content}' WHERE $NOTE_ID = ${note.noteId}"
        db.execSQL(query)
        db.close()
    }
    ...
    fun deleteNote(id: Int) {
        val db = this.writableDatabase
        db.delete(NOTE_TABLE, "$NOTE_ID = ?", arrayOf(id.toString()))
        db.close()
    }
}